        ...
        Discharging)
            ...
            for lvl in "${low_thresholds[@]}"; do
                if (( battery_percentage == lvl )) && [[ ! -f /tmp/.notified_low_$lvl ]]; then
                    ...
                    case $lvl in
                        20) msg="Battery at $battery_percentage%. This is your early warning tweet 📣" ;;
                        15) msg="Battery at $battery_percentage%. It's begging now, plug it in 🙏" ;;
                        10) msg="Battery at $battery_percentage%! Final chapter — save your work 🚨" ;;
                    esac
        ...
        Charging|NotCharging|Unknown)
            ...
            for lvl in "${unplug_thresholds[@]}"; do
                if (( battery_percentage == lvl )) && [[ ! -f /tmp/.notified_unplug_$lvl ]]; then
                    ...
                    case $lvl in
                        85) msg="Battery at $battery_percentage%. The prophecy says unplug at 85% 🧙‍♂️✨" ;;
                        90) msg="Battery at $battery_percentage%. More charging won't make it smarter 😅" ;;
                        95) msg="Battery at $battery_percentage%. Give the charger a break 😌" ;;
                        100) msg="Battery 100%. I'm full bro, unplug now 😵" ;;
                    esac
